...
public class 사전 {
	...
	String name;
	String[] ko = new String[4];
	String[] target = new String[ko.length];
	int cnt = 2;
...
//			result = "X"; // X라는 단어가 있을 수도 있기에 null이 더 적당할 수 있음
			result = null;
		}
	...
	*/
	 ...
	 * 
	 * 매소드 실행 시 단어 추가 됨
	 * cnt가 ko.length보다 클 경우 단어 추가 안 됨 출력
	 * 
	 * 전달인자 : String 추가할 단어
	 * 리턴타입 : X
	*/
	void join(String word, String target) {
		...
		if(cnt < ko.length) {			
			...
			System.out.println("'" + word + " - " + target +"' 단어가 " + this.name +"에 추가되었습니다. ");
			
			cnt++;
		} else {
			System.out.println("사전에 들어갈 수 있는 단어의 최대 크기는 " + ko.length +"개 입니다. ");
		}
	}
